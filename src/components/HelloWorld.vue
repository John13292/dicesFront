<template>
  <div class="hello">
    <div class="container">
      <div class="row">
        <div class="col-md-4" v-for="(image, imageId) in images" :key="imageId">
          <div class="card mb-4 box-shadow">
            <img :src="image.image" class="card-img-top" >
            <div class="card-body">
              <p class="card-text" style="border: red"> </p>
            </div>
          </div>
        </div>
      </div>
  ...
  methods: {
    generatorEffects () {
      ...
      for (let index = 0; index < 3; index++) {
        const numberNegativeEffect = Math.floor(Math.random() * (maxNegativeEffects - min + 1) + min)
        this.historyNegativeEffects.push(this.negativeEffects[numberNegativeEffect].effectText)
        const numberPositiveEffect = Math.floor(Math.random() * (maxPositiveEffects - min + 1) + min)
